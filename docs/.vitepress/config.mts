    ...
    items: [
      ...
      { text: 'TL;DR', link: '/getting-started/tl-dr' },
      {
        ...
        items: [
          ...
          //
          // The basics exist to show how Scrollcase works at all: one demo goes end to end, from an
          // empty directory to a box built, verified and run, and the other does only the last part
          // on a box somebody else signed. They happen to be Python because something had to be,
          // and saying so in the navigation would offer the reader a fact they have no use for yet
          // — which is why no runtime appears at this level.
          //
          // The rest are real workloads, and there the runtime is the point: it is what a reader is
          // choosing between when they wonder whether a box can hold their thing. So the runtimes
          // are group headings, each a peer, none a default, with no suffix on any entry.
          //
          ...
          // the runtimes to top level beside the basics, which read as though the basics were a
          ...
          // choice of wording at a single level managed to say.
          { text: 'The basics', items: [
            { text: 'End-to-end demo', link: '/demos/box-dev-demo' },
            { text: 'How to run a box', link: '/demos/box-run-demo' },
          ]},
          { text: 'Real usecases', items: [
            ...
            { text: 'native', collapsed: true, items: [
