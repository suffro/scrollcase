...
pinned file, lock, commit, sign and build. Longer than the [sentiment demo](/v2/demos/sentiment-demo):
...
[split scroll](/v2/reference/scroll#one-box-several-targets) — just one directory:
...
[`scrollcase refresh`](/v2/reference/cli#refresh) moves those digests after a reviewed change.
...
[split scroll](/v2/reference/scroll#one-box-several-targets): one base carrying the identity, the asset,
...
one ([why that is not automatic](/v2/guides/troubleshooting#running-a-box)). Its `entrypoint.py` is byte
