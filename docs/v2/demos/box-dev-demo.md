...
custody — see [Signing & Key Custody](/v2/guides/signing-and-custody).
:::
...
- Want only to verify and execute an already-built box? Try the [Box-run demo](/v2/demos/box-run-demo).
- To create real project metadata, targets, assets, and execution settings, use
  [`scrollcase new scroll`](/v2/reference/cli#new).
- `doctor` and `audit` are intentionally outside this short demo; see the complete
  [Quickstart](/v2/getting-started/quickstart) and [CLI reference](/v2/reference/cli).
- To run the result from an application, start with the generated templates and the
  [Library APIs reference](/v2/reference/api).
