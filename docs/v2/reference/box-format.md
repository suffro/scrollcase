...
- **Links only where they are provably safe.** A symbolic link is carried when its target is
  ...
  *through* one. Windows boxes carry no links at all, because creating one there needs elevation.
  ...
  duplicates of its own bytes. See [Design decisions](/v2/concepts/design-decisions).
...
[distribution is deliberately out of scope](/v2/concepts/design-decisions).
...
URL. See [Distributing Boxes](/v2/guides/distributing-boxes).
