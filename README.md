...
### Pack entire environments as self-contained boxes
...
- You give that box to someone else. They unpack it and run it — **that's it!** <spacer /> **Nothing to install**: no interpreter, no <small>pip install</small>, no compiler, no Docker, **no dependencies to maintain**.
...
which machine it runs on; the runtime says what starts when it runs.
...
→ [Scroll reference](https://scrollcase.dev/reference/scroll)
