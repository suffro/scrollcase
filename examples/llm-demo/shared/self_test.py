...
`"rome" in answer.lower()` tests what the box is for; an exact sentence would build a guard
...
answer, statistics = generate("What is the capital of Italy?")

assert answer.strip(), "the model produced no output"
assert "rome" in answer.lower(), f"unexpected answer: {answer!r}"
